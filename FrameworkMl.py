...
multichoice = st.sidebar.radio( "🌟 **Select Your Role**", ["🛍️ I'm a Valued Customer", "📈 I'm a Visionary  Business Owner"])

if multichoice == "🛍️ I'm a Valued Customer":
...
if multichoice == "📈 I'm a Visionary  Business Owner":
    st.markdown("""
        <style>
            .stApp {
    ...
    if selection == "About":
            ...
            st.title("Delighted To Have You All")
            st.markdown("""
                :sparkles: **Welcome to SAKS ML Framework!** :sparkles:

                ## Mentor:
                - :heart: **BALACHANDAR KASILINGAM**

                ## Overview:
                Our **Machine Learning Framework** is designed to simplify and accelerate the process of building, 
                training, and deploying machine learning models. It is:

                - **User-friendly**: Designed for both beginners and experts.
                - **Scalable**: Tailored to handle projects of any size.
                - **Business-Centric**: Perfect for businesses of all sizes to harness the power of AI effectively. :robot:

                ### Key Features:
                - :white_check_mark: **Easy to Use**: No need for complex configurations.
                - :zap: **Automation**: Streamline your workflows effortlessly.
                - :recycle: **Flexibility**: Tailored to meet diverse business needs.

                ### Benefits for Clients:
                - :moneybag: **Cost Efficiency**: Optimized for lower computational costs without sacrificing accuracy.
                - :hourglass_flowing_sand: **Reduced Development Time**: Speeds up the entire ML workflow, saving resources.
                - :clap: **Scalability**: Adapts to business growth, handling small datasets to large-scale operations.

                #### Ready to dive in? Let's create something amazing with ML! :rocket:
            """)
    if selection == "Prediction":
        st.header("Prediction Starts Here")
        data = st.file_uploader("Choose A File", type=["csv", "xlsx", "txt"])
        ...
        if data is not None:
            if data.name.endswith('.csv'):
                ...
                option = st.sidebar.radio("Choose the column:", ["DATASET", "SHAPE", "STATISTICS"], index=0)

                if option == "DATASET":
                    st.write(df)
                elif option == "SHAPE":
                    st.write(f"The shape of dataset {df.shape}")
                elif option == "STATISTICS":
                    st.write(df.describe())

                st.sidebar.subheader("Exploratory Data Analysis")
                option1 = st.sidebar.radio("Choose the analysis:", ["UNIVARIATE", "BIVARIATE", "MULTIVARIATE"])
                ...
                if option1 == "UNIVARIATE":
                    ...
                    col1, col2 = st.selectbox("Select column 1", df.columns), st.selectbox("Select column 2",
                                                                                           df.columns)
                    fig, ax = plt.subplots()
                ...
                elif option1 == "MULTIVARIATE":
                    sns.pairplot(df)
                    st.pyplot()

                st.sidebar.subheader("Handle Missing Values")
                option3 = st.sidebar.checkbox("Select to clear target column")
                if option3:
                    target_column1 = st.selectbox("Select the target column", df.columns)
                    if target_column1:
                        feature_column = [col for col in df.columns if col != target_column1]
                        selected_feature = st.multiselect("Select features", feature_column)
                        if selected_feature:
                            ...
                            if len(numerical_cols) > 0:
                                num_strategy = st.selectbox("Choose a strategy for numerical columns",
                                                            ["mean", "median", "constant"])
                                num_imputer = SimpleImputer(
                                    strategy=num_strategy) if num_strategy != "constant" else SimpleImputer(
                                    strategy=num_strategy, fill_value=float(
                                        st.text_input("Enter the constant value for numerical columns", value="0")))
                                X[numerical_cols] = num_imputer.fit_transform(X[numerical_cols])
                                st.write(f"Numerical columns imputed using `{num_strategy}` strategy.")

                            if len(categorical_cols) > 0:
                                cat_strategy = st.selectbox("Choose a strategy for categorical columns",
                                                            ["most_frequent", "constant"])
                                cat_imputer = SimpleImputer(
                                    strategy=cat_strategy) if cat_strategy != "constant" else SimpleImputer(
                                    strategy=cat_strategy,
                                    fill_value=st.text_input("Enter the constant value for categorical columns",
                                                             value=""))
                                X[categorical_cols] = cat_imputer.fit_transform(X[categorical_cols])
                                st.write(f"Categorical columns imputed using `{cat_strategy}` strategy.")

                            st.subheader("Dataset After Imputation")
                            st.dataframe(X)

                            st.sidebar.subheader("Encoding")
                            encoding_strategy = st.sidebar.radio("Choose an encoding strategy",
                            ...
                            if encoding_strategy == "Label Encoding":
                                ...
                                for col in categorical_cols:
                                    mapping = {value: st.number_input(f"Map `{value}` in `{col}` to:", value=0,
                                                                      key=f"{col}_{value}") for value in
                                               df[col].unique()}
                                    df[col] = df[col].map(mapping)
                            ...
                            if algorithm == "Binary Class":
                                ...
                                if r == 'Logistic Regression':
                                    ...
                                    if all(value != "" for value in user_input.values()):
                                        user_input_df = pd.DataFrame(user_input, index=[0])
                                        user_input_encode = pd.get_dummies(user_input_df)
                                        user_input_encode = user_input_encode.reindex(columns=X_encode.columns,
                                                                                      fill_value=0)
                                        prediction = selected_model.predict(user_input_encode)
                                        st.write(f" the predict model is :{prediction[0]}")
                                        st.success("🚀 Boom! Here's your final prediction!")
                                ...
                                if r == 'SVC Algorthim':
                                    ...
                                    if all(value != "" for value in user_input1.values()):
                                        user_input1_df = pd.DataFrame(user_input1, index=[0])
                                        user_input1_encode = pd.get_dummies(user_input1_df)
                                        user_input1_encode = user_input1_encode.reindex(columns=X_encode.columns,
                                                                                        fill_value=0)
                                        prediction = selected_model.predict(user_input1_encode)
                                        st.write(f" the predict model is :{prediction[0]}")
                                        st.success("🚀 Boom! Here's your final prediction!")
                            if algorithm == "Multi Class":
                                r1 = st.selectbox("choose the algorthim:",
                                                  ["Desicion tree Classifier", "Random Forest Classifier",
                                                   "Xgclassifier",
                                                   "Knn Classifier"])
                                X_encode = pd.get_dummies(X)
                                original_columns = X_encode.columns
                                ...
                                if r1 == "Desicion tree Classifier":
                                    ...
                                    if all(value != "" for value in user_input2.values()):
                                        user_input2_df = pd.DataFrame(user_input2, index=[0])
                                        user_input2_encode = pd.get_dummies(user_input2_df)
                                        user_input2_encode = user_input2_encode.reindex(columns=X_encode.columns,
                                                                                        fill_value=0)
                                        prediction = selected_model.predict(user_input2_encode)
                                        st.write(f" the predict model is :{prediction[0]}")
                                        st.success("🚀 Boom! Here's your final prediction!")
                                    ...
                                    rand_pred_str = rand_pred.astype(str)
                                    unique_labels = sorted(set(Y_test_str) | set(rand_pred_str))
                                    rand_presicion = precision_score(Y_test_str, rand_pred.astype(str),
                                                                     average="weighted",
                                                                     labels=unique_labels, zero_division=0)
                                    rand_recall = recall_score(Y_test_str, rand_pred.astype(str), average="weighted",
                                                               labels=unique_labels, zero_division=0)
                                    ...
                                    if all(value != "" for value in user_input3.values()):
                                        user_input3_df = pd.DataFrame(user_input3, index=[0])
                                        user_input3_encode = pd.get_dummies(user_input3_df)
                                        user_input3_encode = user_input3_encode.reindex(columns=X_encode.columns,
                                                                                        fill_value=0)
                                        prediction = selected_model.predict(user_input3_encode)
                                        st.write(f" the predict model is :{prediction[0]}")
                                        st.success("🚀 Boom! Here's your final prediction!")
                                    ...
                                    knn_pred_str = knn_pred.astype(str)
                                    unique_labels = sorted(set(Y_test_str) | set(knn_pred_str))
                                    knn_presicion = precision_score(Y_test_str, knn_pred.astype(str),
                                                                    average="weighted",
                                                                    labels=unique_labels, zero_division=0)
                                    knn_recall = recall_score(Y_test_str, knn_pred.astype(str), average="weighted",
                                                              labels=unique_labels, zero_division=0)
                                    ...
                                    if all(value != "" for value in user_input4.values()):
                                        user_input4_df = pd.DataFrame(user_input4, index=[0])
                                        user_input4_encode = pd.get_dummies(user_input4_df)
                                        user_input4_encode = user_input4_encode.reindex(columns=X_encode.columns,
                                                                                        fill_value=0)
                                        prediction = selected_model.predict(user_input4_encode)
                                        st.write(f" the predict model is :{prediction[0]}")
                                        st.success("🚀 Boom! Here's your final prediction!")
                                    ...
                                    if all(value != "" for value in user_input5.values()):
                                        user_input5_df = pd.DataFrame(user_input5, index=[0])
                                        user_input5_encode = pd.get_dummies(user_input5_df)
                                        user_input5_encode = user_input5_encode.reindex(columns=X_encode.columns,
                                                                                        fill_value=0)
                                        prediction = selected_model.predict(user_input5_encode)
                                        st.write(f" the predict model is :{prediction[0]}")
                                        st.success("🚀 Boom! Here's your final prediction!")
                                ...
                                if model_choice == "Lasso Regression":
                                    ...
                                    for feature in selected_feature:
                                        ...
                                        if all(value != "" for value in user_input8.values()):
                                            user_input8_df = pd.DataFrame(user_input8, index=[0])
                                            user_input8_df = user_input8_df.apply(pd.to_numeric, errors='coerce')
                                            user_input8_df = user_input8_df.reindex(columns=X_train.columns,
                                                                                    fill_value=0)
                                            prediction = selected_model.predict(user_input8_df)
                                            st.write(f" the predicted{target_column1} value is :{prediction[0]:,.2f}")
                                            st.success("🚀 Boom! Here's your final prediction!")
                                if model_choice == "Ridge Regression":
                                    ...
                                    for feature in selected_feature:
                                        ...
                                        if all(value != "" for value in user_input9.values()):
                                            user_input9_df = pd.DataFrame(user_input9, index=[0])
                                            user_input9_df = user_input9_df.apply(pd.to_numeric, errors='coerce')
                                            user_input9_df = user_input9_df.reindex(columns=X_train.columns,
                                                                                    fill_value=0)
                                            prediction = selected_model.predict(user_input9_df)
                                            st.write(f" the predicted{target_column1} value is :{prediction[0]:,.2f}")
                                            st.success("🚀 Boom! Here's your final prediction!")
                            if algorithm == "Non linear Model":
                                choices = st.selectbox("Choose the algorthim:",
                                                       ["Desicion Tree Regresssor", " Random forest Regressor",
                                                        "knn Regressor",
                                                        "Xg boost regressor "])
                                if choices == "Desicion Tree Regresssor":
